...
int main() {
    ...
    ([&db](const crow::request& req){
        ...
        if(user) {
            ...
            res["redirect"] = "/" + ((user->getRol() == "Coordinacion") ? std::string("coordinator") : (user->getRol() == "Tutor" ? std::string("tutor") : std::string("student")));
    ...
    CROW_ROUTE(app, "/api/users/add").methods(crow::HTTPMethod::POST)
    ([&db](const crow::request& req){
        auto x = crow::json::load(req.body);
        if(!x) return crow::response(400);
    ...
    CROW_ROUTE(app, "/api/assign").methods(crow::HTTPMethod::POST)
    ([&db](const crow::request& req){
        auto x = crow::json::load(req.body);
        if(!x) return crow::response(400);
    ...
    // Send
    CROW_ROUTE(app, "/api/chat/send").methods(crow::HTTPMethod::POST)
    ([&db](const crow::request& req){
        auto x = crow::json::load(req.body);
        if(!x) return crow::response(400);
    ...
    ([&db](const crow::request& req){
        ...
        if(!fromStr || !toStr) return crow::response(400);
